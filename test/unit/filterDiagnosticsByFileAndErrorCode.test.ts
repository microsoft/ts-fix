...
import { Diagnostic, DiagnosticCategory, DiagnosticMessageChain, SourceFile } from "typescript";

const default_codes = [];

interface testFileSource extends Diagnostic {
    fileName: string;
}
interface testDiagnostic {
    file: testFileSource | undefined;
    code: number;
...
function makeDiagnostic(code: number, fileName?: string, file?: testDiagnostic): Diagnostic {
    return {
        category: 1,
        code: code,
        file: undefined,
...
test("filterDiagnosticsByFileAndErrorCode_noErrorsInOpt_OnePathNoMathchingFiles", () => {
    const testProperty = { 'file.fileName': 'a.ts' }
    const testDiagnostic = makeDiagnostic(111, "a.ts");
    Object.assign(testDiagnostic, testProperty);
    const testDiagnostic1 = makeDiagnostic(222, "a.ts");
    Object.assign(testDiagnostic1, testProperty);
    const testDiagnostic2 = makeDiagnostic(333, "a.ts");
    Object.assign(testDiagnostic2, testProperty);
    const originalDiagnostics = [[testDiagnostic, testDiagnostic1, testDiagnostic2]];
